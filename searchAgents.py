...
class BidirectionalFoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        # Define your food state
        FoodList = self.foodGrid.asList()
        # Define your initial state
        self.start = (self.init_pos, FoodList)
    ...
    def getGoalStates(self):
        ...
        FoodList = self.foodGrid.asList()
        # add food state into the goal state since they need to be visited
        for fo in FoodList:
            goalStates.append((fo, []))
        
        return goalStates

    def isGoalState(self, state):
        # if there are no other unvisited food states, then it achieved the goal state
        goal_achieved = state[1].count() == 0
        """You code here for Task 3:"""
    ...
    def getSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            dx, dy = Actions.directionToVector(direction)
            # the first attribute of each states contains the information of coordinate
            x_coordinate, y_coordinate = state[0]
            # next state's coordinate will be decided by current coordinate and dx, dy
            x_coordinate_next = int(x_coordinate + dx)
            y_coordinate_next = int(y_coordinate + dy)

            # if the coordinate of next state is not wall
            if not self.walls[x_coordinate_next][y_coordinate_next]:
                tempState = (x_coordinate_next, y_coordinate_next)
                # set the cost to be uniform
                cost = 1
                # the rest food state is simply remove the visted food state  from the rest of the food states
                restFoodState = state[1].copy()
                # if temp state is in the list of rest food, it means that we will visit another food state next
                if(tempState in restFoodState):
                    # so we remove the next food from the list of rest food
                    restFoodState.remove(tempState)
                successors.append(((tempState, restFoodState), direction, cost))
    ...
    def getBackwardsSuccessors(self, state):
        ...
        # almost same as getSuccessors functions, only some attribute need to be modified
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            dx, dy = Actions.directionToVector(direction)
            x_coordinate, y_coordinate = state[0]
            # next state's coordinate will be decided by current coordinate and dx, dy
            ...
            if not self.walls[x_coordinate_next][y_coordinate_next]:
                ...
                backwardAction = Actions.reverseDirection(direction)

                # the food grids are at the corner
                FoodState = Grid(self.foodGrid.width, self.foodGrid.height)
                for (tempx, tempy) in state[1]:
                    # mark the food grid
                    FoodState[tempx][tempy] = True
                FoodState[x_coordinate][y_coordinate] = self.foodGrid[x_coordinate][y_coordinate]
                nextFood = FoodState.asList()
                successors.append(((tempState, nextFood), backwardAction, cost))
    ...
    def getCostOfActions(self, actions):
        ...
        x_coordinate, y_coordinate = self.getStartState()[0]
        for action in actions:
            dx, dy = Actions.directionToVector(action)
            x_coordinate=int(x_coordinate+dx)
            y_coordinate=int(y_coordinate+dy)

            # if the next state is wall
            if self.walls[x_coordinate][y_coordinate]:
                return 999999
            cost = cost+1
        return cost
...
def bidirectionalFoodProblemBackwardsHeuristic(state, problem):
    "*** YOUR CODE HERE for Task 3 ***"
    distance = 0
    initialList = problem.getStartState()[1]
    position, foodList = state
    for food in initialList:
